...
Nodes: memories + entities, both persisted in Engram's local SQLite mirror
(kept in sync with Supermemory content by pipeline.ingest_memory). Edges:
memory→memory typed relationships and memory→entity mentions. Served as JSON
for the frontend force-graph and for `find_related` (direct relationship
neighbors, strongest edges first).
"""
